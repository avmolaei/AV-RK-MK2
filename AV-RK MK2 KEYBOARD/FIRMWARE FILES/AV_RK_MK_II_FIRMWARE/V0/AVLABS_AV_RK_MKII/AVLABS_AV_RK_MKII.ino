...
void loop() {
  for (uint8_t i = 0; i < NUM_KEYS_NUMPAD; i++) {
    pressKey(NUMPADMAP[i].expander, NUMPADMAP[i].pin, NUMPADMAP[i].key, keyDelay);
  }
  /*
  for (uint8_t i = 0; i < NUM_KEYS_LEFT; i++) {
 ...
 * @param key the keycode corresponding to the key in the keymap 
 * @param delay the delay after which one keystroke gets spammed.
*/
void pressKey(PCA9505_06* expander, uint8_t pin, uint8_t key, uint8_t delay) {
  if (expander->digitalRead(pin) == HIGH) {
    if (key != lastKey) {
      lastPressTime = millis();
      lastKey = key;
      Keyboard.press(key);
    }
    else if ((millis() - lastPressTime) > delay) {
      Keyboard.write(key);
    }
  }
  else {
